...
uint32_t test_value ;
uint8_t state = 0;
#define TEST_ADDRESS 0x1000  // 测试地址，避开Flash起始区域
//uint32_t num = 0;           // 要保存的变量
// 最简单的测试 - 只实现基本的保存和读取
/**
  * @brief  W25Qxx Flash存储器基础测试函数
  * @note   该函数执行Flash的完整读写测试流程，包括初始化、读取、擦除、写入和验证操作
  * @retval None
  */
void OSPI_W25Qxx_BasicTest(void)
{
    // 初始化OSPI接口和W25Qxx Flash存储器
    OSPI_W25Qxx_Init();

    // 状态机控制：首次执行时备份原始数据，后续执行时使用测试值
    if(state == 0)
    {
        // 从测试地址读取当前数据并保存到read_back变量
        OSPI_W25Qxx_ReadBuffer((uint8_t*)&read_back, TEST_ADDRESS, sizeof(read_back));
        // 将读取的数据保存为测试基准值
    ...
    // 擦除目标扇区（擦除操作是Flash写入的必要前置步骤）
    OSPI_W25Qxx_SectorErase(TEST_ADDRESS);

    // 将测试值写入到目标地址
    OSPI_W25Qxx_WriteBuffer((uint8_t*)&test_value, TEST_ADDRESS, sizeof(test_value));

    // 从目标地址读取刚写入的数据到read_back变量，用于后续验证
    OSPI_W25Qxx_ReadBuffer((uint8_t*)&read_back, TEST_ADDRESS, sizeof(read_back));
